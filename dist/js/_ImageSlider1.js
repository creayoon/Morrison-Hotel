(function($,window){
	...
	_ImageSlider.prototype = {
		init:function(selector){
			...
			this.imgLength = this.$images.length-3 // 클론갯수 빼준 것
		...
		initImage:function(){
			...
			this.totalW = this.imageW * this.imgLength; 
		...
		prev : function(){
			...
			var self = this;
			if(this.idx <= 0){
				this.idx = this.imgLength; 

				this.wrapper.css({ 
					transition : 'none',
					transform : "translate("+((-this.firstImagePos)+(-this.imageW)*this.imgLength)+"px, 0px)"
				});
			...
			// 바뀐 현재위치 업데이트 하려고
			this.leftMove = (-this.imageW)*this.idx;
			// console.log(this.leftMove); // 현재위치 업데이트 안하면 
		...
		next : function(){
			...
			$(".sliderDot li").removeClass("active");
			$(".sliderDot li").eq(this.idx-(this.imgLength-1)).addClass("active"); // 첫번째 index 안나오는거 숫자 맞춰주려고
